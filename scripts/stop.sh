...
lsof -ti:8000 2>/dev/null | xargs kill -9 2>/dev/null && echo "  ✓ Stopped MemoryStack (port 8000)" || echo "  • MemoryStack not running"
...
if [ -f "proxy.pid" ]; then
    kill -9 $(cat proxy.pid) 2>/dev/null && rm proxy.pid
fi
